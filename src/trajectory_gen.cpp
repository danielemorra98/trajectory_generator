...
TrajGen::TrajGen()
  ...
  measure_tracking_timer_ =
      nh_.createTimer(ros::Duration(1.0 / kExecLoopRate_),
                      &TrajGen::measureTracking, this);
}

TrajGen::~TrajGen() {}

void TrajGen::measureTracking(const ros::TimerEvent& time) {
...
void TrajGen::run() {
  ...
  if((autopilot_helper_.waitForSpecificAutopilotState(
      ...
      std::cout << "Autopilot could not be forced to off during take off.";
...
//       if(((autopilot_helper_.getCurrentAutopilotState() ==
//                    autopilot::States::VELOCITY_CONTROL)))
//           std::cout << "Autopilot did not switch to velocity control correctly.";
//       break;
//     }
//     ros::spinOnce();
//     command_rate.sleep();
...
//   if((autopilot_helper_.waitForSpecificAutopilotState(
//       autopilot::States::HOVER, 10.0, kExecLoopRate_)))
//       std::cout << "Autopilot did not switch back to hover correctly.";
  ...
  if((autopilot_helper_.waitForSpecificAutopilotState(
      ...
      std::cout << "Autopilot did not switch to trajectory control because of go to pose "
         "action correctly.";
  ...
  if(
      (autopilot_helper_.getCurrentReferenceState().position - position_cmd)
          .norm() < 0.01)
      std::cout << "Go to pose action did not end up at the right position.";

  if(autopilot_helper_.getCurrentReferenceHeading() - heading_cmd <
              0.01)
      std::cout << "Go to pose action did not end up at the right heading.";
  ...
  end_state.position = Eigen::Vector3d(0.0, 0.0, 1.5);
  ...
  if(autopilot_was_in_reference_control_mode)
      std::cout << "Autopilot did not switch to reference control correctly.";
  ...
  if(autopilot_helper_.waitForSpecificAutopilotState(
      ...
      std::cout << "Autopilot did not switch to hover after being forced to hover.";
  ...
  if(autopilot_helper_.waitForSpecificAutopilotState(
      ...
      std::cout << "Autopilot did not switch to land after sending land command within "
         "timeout.";

  // Wait for autopilot to go to off
  if(autopilot_helper_.waitForSpecificAutopilotState(
      autopilot::States::OFF, 10.0, kExecLoopRate_))
      std::cout << "Autopilot did not switch to off after landing within timeout.";
  ...
  while (ros::ok()) {
    ...
    if ((ros::Time::now() - start_sending_cont_cmds) > ros::Duration(0.5)) {
      if((autopilot_helper_.getCurrentAutopilotState() ==
          ...
          std::cout << "Autopilot did not switch to command feedthrough correctly.";
      break;
    }
    ros::spinOnce();
    command_rate.sleep();
